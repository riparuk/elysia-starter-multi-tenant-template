...
describe('Auth Module Tests (via better-auth)', () => {
	describe('POST /auth/api/sign-in/email', () => {
		it('should reject invalid login credentials', async () => {
			const SignInResponse = await (tClient as any).auth.api[
				'sign-in'
			].email.post({
	...
	describe('GET /auth/api/get-session', () => {
		it('should return null when no session exists', async () => {
			const GetSessionResponse = await (tClient as any).auth.api[
				'get-session'
			].get()
	...
	describe('POST /auth/api/sign-in/email', () => {
		it('should sign up and sign in a user', async () => {
			...
			const SignUpResponse = await (tClient as any).auth.api[
				'sign-up'
			].email.post({
				email: testEmail,
			...
			const cookieHeader =
				SignInResponse.response?.headers.get('set-cookie')
			if (cookieHeader) {
